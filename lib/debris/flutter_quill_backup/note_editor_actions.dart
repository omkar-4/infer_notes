...
mixin NoteEditorActions on NoteEditorState {
  Future<void> createNewNote() async {
    ...
    String baseName = 'Untitled Note';
    String ext = '.md';
    String newPath = '$vaultPath${Platform.pathSeparator}$baseName$ext';
    int counter = 1;
    
    while (File(newPath).existsSync()) {
      newPath = '$vaultPath${Platform.pathSeparator}$baseName $counter$ext';
      counter++;
    }
    ...
    if (isNoteOpen && currentFilePath != null) {
      ...
      if (oldPath.endsWith('.md')) {
        if (quillController != null) {
          ...
          contentToSave = jsonEncode(quillController!.document.toDelta().toJson());
        }
      } else {
        contentToSave = controller.text;
      }
      
      File(oldPath).writeAsString(contentToSave).then((_) {
  ...
  Future<void> saveNote() async {
    if (!isNoteOpen || currentFilePath == null) return;
    ...
    try {
      ...
      await File(currentFilePath!).writeAsString(contentToSave);
